...
const stats: StatItem[] = [
  ...
  { value: 4.9, suffix: "★", label: "Average Rating", starColor: "#B65B3A" },
...
export default function StatsSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative -mt-10 overflow-hidden bg-[#EEE4D6] px-5 pb-10 pt-16 sm:px-8"
      ...
      <div className="mx-auto max-w-7xl">
        <div className="relative">
          <svg className="pointer-events-none absolute left-0 top-1/2 hidden h-[2px] w-full -translate-y-1/2 lg:block" viewBox="0 0 1200 2" preserveAspectRatio="none">
            <line ref={lineRef} x1="0" y1="1" x2="1200" y2="1" stroke="rgba(182,91,58,0.3)" strokeWidth="1" />
          </svg>
          ...
          >
            {stats.map((item, index) => (
              <article
                ...
                className={`relative py-3 ${index !== stats.length - 1 ? "lg:after:absolute lg:after:right-0 lg:after:top-1/2 lg:after:h-14 lg:after:w-px lg:after:-translate-y-1/2 lg:after:bg-[#2A2019]/15" : ""}`}
              >
                <p className={`${item.large ? "text-5xl" : "text-4xl"} ${item.mono ? "font-mono" : "font-display"} font-extrabold leading-none text-[#241B16]`}>
                  <span
                    ref={(element) => {
                ...
                <p className="mt-2 text-[13px] font-normal text-[#6D5A4E]">{item.label}</p>
              </article>
            ))}
          </div>
        </div>

        <div className="mt-10 space-y-2 overflow-hidden border-t border-[#2A2019]/10 pt-4">
          {[0, 1].map((row) => (
            <div key={`marquee-${row}`} className="flex min-w-max animate-marquee gap-10 whitespace-nowrap font-mono text-[11px] tracking-[0.12em] text-[#9C8778]">
