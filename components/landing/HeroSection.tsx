...
function FloatingEventCard({ data, top, right, rotate, y }: FloatingEventCardProps) {
  ...
  return (
    <motion.article
      className="absolute w-[280px] rounded-[20px] border border-[#E9DDCF] bg-[rgba(255,251,245,0.82)] p-5 text-[#241B16] backdrop-blur-xl"
      style={{ top, right, y, rotate, rotateX, rotateY, transformStyle: "preserve-3d" }}
      whileHover={{
        scale: 1.03,
        borderColor: "rgba(182,91,58,0.45)",
    ...
    >
      <div className="mb-3 flex items-center justify-between">
        <span className="rounded-full bg-[#B65B3A]/10 px-2.5 py-1 text-[11px] font-semibold text-[#9E4E31]">{data.category}</span>
        {data.isToday ? (
      ...
      <h3 className="mb-3 text-base font-semibold text-[#241B16]">{data.event_name}</h3>

      <div className="space-y-1.5 text-xs text-[#6D5A4E]">
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <button type="button" className="text-xs font-medium text-[#B65B3A] hover:text-[#8E4428]" onClick={() => window.location.assign("/events") }>
          Book -&gt;
        </button>
      </div>
    </motion.article>
...
export default function HeroSection() {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-[#F8F4EE] pt-24 text-[#241B16]">
      <div
        aria-hidden
        className="pointer-events-none absolute -left-[100px] -top-[200px] h-[700px] w-[700px] animate-float rounded-full blur-[40px]"
        style={{
          background: "radial-gradient(circle, rgba(182,91,58,0.16) 0%, transparent 70%)",
        }}
      />
      <div
        aria-hidden
        ...
        style={{
          ...
          background: "radial-gradient(circle, rgba(196,125,80,0.12) 0%, transparent 70%)",
        }}
      />
      <div
        aria-hidden
        ...
        style={{
          ...
          animationDelay: "2s",
          background: "radial-gradient(circle, rgba(140,68,40,0.08) 0%, transparent 70%)",
        }}
      />
      <div
        aria-hidden
        ...
        style={{
          backgroundImage: "radial-gradient(rgba(55,42,34,0.12) 1px, transparent 1px)",
        ...
        <div className="col-span-12 flex items-center py-12 lg:col-span-7 lg:py-0">
          <div className="space-y-7">
            <div ref={preheadingRef} className="flex items-center gap-3">
              <span className="h-[2px] w-6 rounded bg-[#B65B3A]" />
              <p className="font-mono text-[11px] tracking-[0.2em] text-[#B65B3A]">THE FUTURE OF EVENT BOOKING</p>
            </div>
            ...
            >
              <span className="block text-[clamp(56px,8vw,100px)] font-extrabold text-[#241B16]">Every Great</span>
              <span
                data-hero-accent="true"
                className="block text-[clamp(64px,9vw,120px)] font-extrabold italic text-[#B65B3A]"
              >
                Moment
              </span>
              <span className="block text-[clamp(56px,8vw,100px)] font-extrabold text-[#241B16]">Starts Here.</span>
            </h1>

            <p
              ref={subheadlineRef}
              className="max-w-[480px] text-[clamp(16px,2vw,20px)] font-normal leading-[1.6] text-[#6D5A4E]"
            >
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                >
                  <Button
                    ...
                    className="relative h-[52px] overflow-hidden rounded-xl bg-[#B65B3A] px-8 text-base font-semibold text-[#FFF8F0] hover:bg-[#9E4E31]"
                  >
                ...
                <Button
                  ...
                  className="h-[52px] rounded-xl border border-[#2E221B]/20 bg-[#FFF8F0]/70 px-6 text-base font-medium text-[#2E221B] hover:bg-[#F1E5D8]"
                >
                  <Play className="h-4 w-4 animate-pulse text-[#9E4E31]" />
                  Watch Demo
                </Button>
              </div>

              <div ref={trustRowRef} className="mt-6 flex flex-wrap items-center gap-3 text-[13px] text-[#6D5A4E]">
                <span>✓ Free to register</span>
                <span className="h-1 w-1 rounded-full bg-[#9C8778]" />
      ...
      <div className="pointer-events-none absolute bottom-8 left-1/2 z-10 -translate-x-1/2">
        <div ref={indicatorRef} className="flex flex-col items-center gap-1.5">
          <ChevronDown className="h-5 w-5 text-[#8D7667]" />
          <p className="font-mono text-xs text-[#8D7667]">Scroll to explore</p>
        </div>
      </div>
      ...
      <Modal isOpen={videoOpen} onClose={() => setVideoOpen(false)} title="Demo Preview">
        <div className="space-y-4 text-sm text-zinc-700">
          ...
          <Button onClick={() => router.push("/events")} className="w-full bg-[#B65B3A] text-[#FFF8F0] hover:bg-[#9E4E31]">
            Go to Events
          </Button>
        </div>
      </Modal>
