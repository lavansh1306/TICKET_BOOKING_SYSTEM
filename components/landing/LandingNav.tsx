...
export default function LandingNav() {
  ...
  const backgroundColor = useTransform(scrollY, [0, 80], ["rgba(248,244,238,0)", "rgba(248,244,238,0.9)"]);
  const borderBottomColor = useTransform(scrollY, [0, 80], ["rgba(35,26,21,0)", "rgba(35,26,21,0.1)"]);
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
          <Link
            href="/"
            className={`font-display text-[18px] font-bold tracking-[0.02em] text-[#231A15] transition-colors duration-300 hover:text-[#B65B3A] ${
              isScrolled ? "text-[#231A15]" : "text-[#2B201A]"
            }`}
          >
            BOOKING_SYSTEM
          </Link>
          ...
          <div className="hidden items-center gap-8 md:flex">
            {sectionLinks.map((link) => (
              <button
                ...
                className="group relative text-sm text-[#3A2D25]/75 transition-opacity duration-300 hover:text-[#231A15]"
              >
                {link.label}
                <span className="absolute -bottom-1 left-0 h-[1px] w-0 bg-[#B65B3A] transition-all duration-300 group-hover:w-full" />
              </button>
            ))}
          </div>
          ...
          <div className="hidden items-center gap-3 md:flex">
            <Link
              href="/auth/login"
              className="inline-flex h-11 items-center justify-center rounded-xl border border-[#2E221B]/30 px-5 text-sm font-medium text-[#231A15] transition-colors duration-300 hover:bg-[#231A15] hover:text-[#FFF9F2]"
            >
              Sign In
            </Link>
            ...
            >
              <Button
                className="h-11 rounded-xl bg-[#B65B3A] px-5 text-sm font-semibold text-[#FFF8F0] hover:bg-[#9E4E31]"
          ...
          <button
            ...
            className="relative z-[60] flex h-11 w-11 flex-col items-center justify-center gap-[5px] rounded-lg border border-[#2E221B]/25 bg-[#FFF8EF]/80 md:hidden"
            onClick={() => setIsMobileOpen((open) => !open)}
          >
            <span ref={topLineRef} className="h-[2px] w-5 rounded bg-[#2E221B]" />
            <span ref={middleLineRef} className="h-[2px] w-5 rounded bg-[#2E221B]" />
            <span ref={bottomLineRef} className="h-[2px] w-5 rounded bg-[#2E221B]" />
          </button>
        </div>
      </motion.nav>
      ...
      <div
        ...
        className="fixed inset-0 z-40 flex items-center justify-center bg-[#F8F1E6] md:hidden"
          ...
          {mobileLinks.map((link, index) => {
            ...
            return (
              <button
                ...
                type="button"
                className="text-2xl font-display font-bold tracking-wide text-[#2E221B]"
                onClick={onClick}
              >
                {link.label}
              </button>
